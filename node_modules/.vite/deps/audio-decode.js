...
function isMp3(buf) {
  ...
  buf[0] === 255 && (buf[1] & 224) === 224 || // no tag
...
var AudioBuffer2 = globalThis.AudioBuffer || AudioBuffer;
...
var decoders = {
  async oga(buf) {
    ...
    if (!decoder) {
      let { OggVorbisDecoder } = await import("./ogg-vorbis-KMNYLEZS.js");
  ...
  async mp3(buf) {
    ...
    if (!decoder) {
      const { MPEGDecoder } = await import("./mpg123-decoder-ZEYFPLMB.js");
  ...
  async flac(buf) {
    ...
    if (!decoder) {
      const { FLACDecoder } = await import("./flac-7YRVE5VO.js");
  ...
  async opus(buf) {
    ...
    if (!decoder) {
      const { OggOpusDecoder } = await import("./ogg-opus-decoder-53DP3PRY.js");
  ...
  async wav(buf) {
    ...
    if (!decode) {
      let module = await import("./node-wav-T6CXKBTD.js");
  ...
  async qoa(buf) {
    ...
    if (!decode) {
      decoders.qoa.decode = decode = (await import("./qoa-format-XRPMGOD4.js")).decode;
    }
    return buf && createBuffer(await decode(buf));
  }
};
function createBuffer({ channelData, sampleRate }) {
  let audioBuffer = new AudioBuffer2({
    sampleRate,
