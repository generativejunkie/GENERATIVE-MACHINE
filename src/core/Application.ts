...
export class Application extends EventEmitter<ApplicationEventMap> {
  ...
  private animate = (): void => {
    ...
    if (this.state.spaceMode && this.state.isPlaying) {
      ...
      // Space mode = Mandala + AUTO integrated
      // Enable both Mandala and AUTO when Space is ON
      if (!this.state.mandalaMode) {
        this.state.mandalaMode = true;
      }
    ...
    // AUTO Mode: Auto-spawn objects (also called by Space mode)
  ...
  /**
   * AUTO Mode: Auto-spawn objects up to 20
   */
  private handleAutoMode(force: boolean = false): void {
    ...
    // All modes limited to max objects setting (default 20)
    const maxObjects = this.instanceManager.getMaxObjects();
    ...
    if ((force || (!this.lastAIObjectSpawn || now - this.lastAIObjectSpawn > objectSpawnInterval)) && this.instanceManager.getAllInstances().length < maxObjects) {
      ...
      if (Math.random() < 0.7) {
        if (highNorm > lowNorm && highNorm > midNorm) {
          materialIndex = 16 + Math.floor(Math.random() * 7);
        } else if (lowNorm > midNorm) {
          materialIndex = Math.floor(Math.random() * 8);
        } else {
          materialIndex = 8 + Math.floor(Math.random() * 8);
        }
      } else {
        materialIndex = Math.floor(Math.random() * MATERIALS.length);
      }

      materialIndex = Math.min(materialIndex, MATERIALS.length - 1);
      this.addInstance(materialIndex);
      this.lastAIObjectSpawn = now;

      if (force) console.log(`🥁 Beat-spawned material #${materialIndex}`);
      else console.log(`🤖 Timer-spawned material #${materialIndex}`);
    }
  }
  ...
  private handleGravityMode(): void {
    const now = Date.now();
    // Random switching every 2-5 seconds
    if (now - this.lastMandalaUpdate > 3000) { // Reuse timer or create new
      this.lastMandalaUpdate = now;

      // Randomly add/remove objects to simulate drifting population
